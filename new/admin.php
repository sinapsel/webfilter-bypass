...
class SkinOutput extends Output {
   ...
   protected function wrap() {
      ...
      $title = $this->title . ( $this->title ? ' : ' : '' ) . 'control panel';
   ...
   ******************************************************************/
   ...
   case '':
      ...
      if ( ! ( $ok = version_compare($phpVersion, '5', '>=') ) ) {
         $error->add('<strong>Proxy</strong> requires at least PHP 5 or greater.');
      }
      ...
      if ( ! ( $ok = function_exists('curl_version') ) ) {
         $error->add('<strong>Proxy</strong> requires cURL/libcurl.');
      }
...
OUT;
      ...
      if ( $error->hasMsg() ) {
         echo '<p><span class="bold error-color">Environment check failed</span>. You will not be able to run proxy until you fix the above issue(s).</p>';
      } else {
         echo '<p><span class="bold ok-color">Environment is okay</span>. Proxy could be used.</p>';
      }
